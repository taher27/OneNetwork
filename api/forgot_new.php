...
if(!empty($_POST['userid'])){
    ...
    $row = mysqli_fetch_row($result_checkuser);
    $msg = "";
    if($row>0){
        ...
        if(!user_already_exists($con ,$userid)){
            ...
            if($sendmail){
                ...
                if(mysqli_query($con, $query)){
                    $url = "http://".$_SERVER['SERVER_NAME'].chop($_SERVER["PHP_SELF"],"api/forgot_neforgot_password.php"        )."/change_password.php?code1=".$code1."&code2=".$code2;
    ...
    $response = array("error" => "true", "message" => "not success", "description" => "user id cannot be empty");
}
