<?php
   ...
   if(isset($_POST["project_id"]) && isset($_POST["user_id"])){
        ...
        if(mysqli_num_rows($result)>0){
            ...
            $query = "UPDATE `applied_users` SET `approved` = 1 WHERE `user_id`='".$_POST["user_id"]."' AND `project_id`='".$_POST["project_id"]."'";
            mysqli_query($con, $query) or die(mysqli_error($con));
            $query = "INSERT INTO `project_members` (`project_id`, `user_id`) VALUES(".$_POST["project_id"].",'".$_POST["user_id"]."')";
            mysqli_query($con, $query) or die(mysqli_error($con));
            $query = "INSERT INTO `notifications` (`owner_id`, `project_id`, `user_id`, `type`, `seen`, `time`) VALUES('".$_POST["user_id"]."', ".$_POST["project_id"].", '".$project_owner."', 'approved_student', 0, ".time().")";
            mysqli_query($con, $query) or die(mysqli_error($con));
